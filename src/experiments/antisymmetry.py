...
import re 
import numpy as np 
from sage.all import pi, floor, sin, ln, cos
...
class AntiSymmetry(Run):
    path = f"{DATA_PATH}/antisymmetry/"
    name = lambda self, **kwargs: f"antisymmetry(p={kwargs['p']},d={kwargs['d']})"
    required = ['p', 'd']
...
if __name__ == "__main__":
    primes = sample_primes(50, 500000, 750000)
    config = Config(
            run_instance    = AntiSymmetry(),
