...
from utils.r_utils import compute_R_sum, compute_R_char_sum

MAIN_PATH = os.path.join("/Users", "jcheigh", "Thesis")
DATA_PATH = os.path.join(MAIN_PATH, "data")

class RCharSum(Run):
    path = f"{DATA_PATH}/r char sum lists/"
    name = lambda self, **kwargs: f"k = {kwargs['k']}, H in 1-{kwargs['H']} R char sum"
    required = ['H', 'k']
    
    def main(self, H: int, k: int):
        return compute_R_char_sum(k, H)

    def validate_inputs(self, H, k):
        return True 

if __name__ == "__main__":
    primes = sample_primes(6, 10000, 20000)
    H = 1000
    config = Config(
            run_instance    = RCharSum(),
